...
class PredictionEngine:
    ...
    def predict_impact(self, file_path: str) -> Prediction:
        ...
        # Sort by confidence
        related_files.sort(key=lambda x: float(x["confidence"]), reverse=True)

        # Calculate overall confidence
        if related_files:
            avg_confidence = sum(float(f["confidence"]) for f in related_files) / len(related_files)
        else:
            avg_confidence = 0.0
    ...
    def predict_risks(self, change_description: str) -> Prediction:
        ...
        if not keywords:
            ...
            query = (
                "SELECT r.description, COUNT(DISTINCT r.session_id) as frequency, "  # nosec B608  # noqa: S608
    ...
    def predict_complexity(self, task_description: str) -> Prediction:
        ...
        if keywords:
            ...
            # WHERE clause shape is generated from fixed fragments; values are bound.
            query = (
                "SELECT t.description, t.status, "  # nosec B608  # noqa: S608
        ...
        if similar_tasks:
            ...
            if completed_tasks:
                avg_duration = sum(t["duration_days"] for t in completed_tasks) / len(
                    completed_tasks
                )
    ...
    def recommend_tests(self, file_paths: list[str]) -> Prediction:
        ...
        if file_paths:
            ...
            query = (
                "SELECT f1.path AS related_to, f2.path AS test_path, "  # nosec B608  # noqa: S608
