...
def format_summary(diff: PackDiff, use_color: bool = True) -> str:
    ...
    if diff.proof_chain_diff and diff.proof_chain_diff.status != "unchanged":
        ...
        if pc.file_hash_changes:
            lines.append(color(f"  • {len(pc.file_hash_changes)} file hash(es) changed", COLOR_YELLOW))
                ...
                for section in crumb_diff.modified_sections:
                    ...
                    if old_content and new_content:
                        ...
                        if len(diff_lines) > 4:  # Skip if only headers
                            for line in diff_lines[2:]:  # Skip diff headers
                                line = line.rstrip()
                                if line.startswith("+"):
                                    lines.append(color(f"        {line}", COLOR_GREEN))
                                elif line.startswith("-"):
                                    lines.append(color(f"        {line}", COLOR_RED))
...
def format_json(diff: PackDiff) -> str:
        ...
        "crumb_diffs": [
            {
                "path": d.path,
                "status": d.status,
                "header_changes": {k: {"old": v[0], "new": v[1]} for k, v in d.header_changes.items()},
    ...
    if diff.proof_chain_diff:
        ...
        result["proof_chain"] = {
            ...
            "count_changes": {k: {"old": v[0], "new": v[1], "delta": v[1] - v[0]} for k, v in pc.count_changes.items()},
            "file_hash_changes": {k: {"old": v[0], "new": v[1]} for k, v in pc.file_hash_changes.items()},
