...
class PackRegenerationHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event: FileSystemEvent) -> None:
        ...
        if self._should_process_event(event):
            self._schedule_regeneration(event.src_path, "modified")
    
    def on_created(self, event: FileSystemEvent) -> None:
        """Handle file creation events."""
        if self._should_process_event(event):
            self._schedule_regeneration(event.src_path, "created")
    
    def on_deleted(self, event: FileSystemEvent) -> None:
        """Handle file deletion events."""
        if self._should_process_event(event):
            self._schedule_regeneration(event.src_path, "deleted")
