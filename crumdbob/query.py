...
class QueryEngine:
    ...
    def _init_patterns(self) -> None:
        ...
        self.patterns = [
            ...
            (
                ...
                lambda m: self._hot_files_query(
                    self._parse_time_period(m.group(1) if m.lastindex else "month")
                ),
            ),
            (
            ...
            (r"(?:show|find|get|list)\s+hot\s+files?", lambda _m: self._hot_files_query()),
            # Task queries
            (
    ...
    def query_sql(self, sql: str) -> QueryResult:
        ...
        if not normalized:
            return QueryResult(
                query=sql, results=[], row_count=0, explanation="SQL error: empty query"
            )
        ...
        try:
            ...
            return QueryResult(query=sql, results=[], row_count=0, explanation=f"SQL error: {e}")
    ...
    def _hot_files_query(self, days: int = 30) -> QueryResult:
        ...
        return QueryResult(
            query=f"SELECT hot files (requested window: {days} days)",
            results=results,
            row_count=len(results),
            explanation=f"Found {len(results)} files changed across multiple recorded sessions",
        )
